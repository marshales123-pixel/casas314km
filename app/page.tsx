...
const amenitiesList = [
  { label: "Seguridad 24 hs", Icon: Shield },
  { label: "Club House", Icon: HomeIcon },
  { label: "Pileta", Icon: Waves },
  { label: "Canchas de fútbol y tenis", Icon: Trophy },
  { label: "Juegos para chicos", Icon: Gamepad2 },
  { label: "Playa semiprivada", Icon: Umbrella },
];
    ...
    <main className="bg-white text-gray-800">
      ...
      <section className="relative flex min-h-[85vh] items-end overflow-hidden">
        ...
        <div className="relative z-10 mx-auto w-full max-w-6xl px-4 pb-20 sm:px-6 lg:px-8">
          <p className="text-xs uppercase tracking-[0.35em] text-teal-300 font-semibold">
            Barrio Privado · Costa Atlántica
          </p>
          <h1 className="mt-4 max-w-3xl text-4xl font-bold leading-tight text-white sm:text-5xl lg:text-6xl lg:leading-[1.1]">
            Casas de mar dentro del barrio
          </h1>
          <p className="mt-5 max-w-xl text-base text-white/75 sm:text-lg leading-relaxed">
            Viví el mar, la naturaleza y la tranquilidad en un entorno privado pensado para descansar.
          </p>
          <div className="mt-9 flex flex-wrap gap-3">
            <Link
              href="/casas"
              className="rounded-full bg-teal-600 px-7 py-3 text-sm font-semibold text-white shadow-lg transition hover:bg-teal-500 hover:shadow-teal-500/30 hover:shadow-xl"
            >
              Ver casas disponibles
            </Link>
            <Link
              href="/propietarios"
              className="rounded-full border border-white/40 px-7 py-3 text-sm font-medium text-white/90 backdrop-blur-sm transition hover:bg-white/10 hover:border-white/70"
            >
              Publicar mi casa
            </Link>
          </div>
      ...
      {/* ── INTRO + AMENITIES ── */}
      <section className="mx-auto max-w-6xl px-4 py-20 sm:py-24 sm:px-6 lg:px-8">
        <div className="grid gap-12 lg:grid-cols-2 lg:items-start">
          <div>
            <p className="text-xs uppercase tracking-[0.3em] text-teal-600 font-semibold">Viví Km 314</p>
            <h2 className="mt-3 text-3xl font-bold text-stone-900 sm:text-4xl leading-snug">
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            {amenitiesList.map(({ label, Icon }) => (
              <div
                key={label}
                className="flex flex-col gap-3 rounded-2xl border border-stone-100 bg-stone-50/80 p-4 transition-all hover:border-teal-200 hover:bg-teal-50/50 hover:-translate-y-0.5"
              >
      ...
      {/* ── BARRIO FOTOS ── */}
      <section className="mx-auto max-w-6xl px-4 pb-20 sm:pb-24 sm:px-6 lg:px-8">
        ...
        <div className="hidden sm:grid sm:grid-cols-2 sm:gap-3 lg:grid-cols-4">
          {homeImages.barrio.map((img, i) => (
            <img
              ...
              className="h-52 w-full rounded-2xl object-cover transition duration-300 hover:scale-[1.02] hover:shadow-lg lg:h-56"
            />
          ))}
        </div>
      </section>
      ...
      <section className="bg-stone-50 py-20 sm:py-24">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div className="grid gap-10 lg:grid-cols-2 lg:items-center">
            <div>
              <p className="text-xs uppercase tracking-[0.3em] text-teal-600 font-semibold">Playa</p>
              <h2 className="mt-3 text-3xl font-bold text-stone-900 sm:text-4xl leading-snug">
            ...
            <div className="hidden sm:grid sm:grid-cols-2 sm:gap-3">
              {homeImages.playa.map((img, i) => (
                <img
                  ...
                  src={img}
                  alt={`Playa Km314 ${i + 1}`}
                  className="h-60 w-full rounded-2xl object-cover transition duration-300 hover:scale-[1.02] hover:shadow-lg"
                />
              ))}
            </div>
          </div>
      ...
      {/* ── AMENITIES FOTOS ── */}
      <section className="mx-auto max-w-6xl px-4 py-20 sm:py-24 sm:px-6 lg:px-8">
        <p className="text-xs uppercase tracking-[0.3em] text-teal-600 font-semibold">Amenities</p>
        <h2 className="mt-3 text-3xl font-bold text-stone-900 sm:text-4xl leading-snug">
        ...
        <div className="mt-8 hidden sm:grid sm:grid-cols-2 sm:gap-3 lg:grid-cols-4">
          {homeImages.amenities.map((img, i) => (
            <img
              ...
              src={img}
              alt={`Amenity Km314 ${i + 1}`}
              className="h-44 w-full rounded-2xl object-cover transition duration-300 hover:scale-[1.02] hover:shadow-lg"
            />
          ))}
        </div>
      </section>
        ...
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
          <div className="mb-10 flex items-end justify-between">
            <div>
              ...
              <h2 className="mt-2 text-3xl font-bold text-stone-900 sm:text-4xl leading-snug">
                Propiedades disponibles
              </h2>
            </div>
            <Link
              href="/casas"
              className="hidden items-center gap-1.5 text-sm font-semibold text-teal-700 hover:text-teal-900 transition md:flex"
            >
              Ver todas
          ...
          <div className="mt-8 text-center md:hidden">
            <Link
              href="/casas"
              className="inline-flex items-center gap-1.5 rounded-full border border-teal-200 bg-teal-50 px-6 py-2.5 text-sm font-semibold text-teal-700 transition hover:bg-teal-100"
            >
              Ver todas las casas →
            </Link>
          </div>
      ...
      <section className="bg-stone-900 py-20 sm:py-24">
        <div className="mx-auto max-w-6xl px-4 text-center sm:px-6 lg:px-8">
          <p className="text-xs uppercase tracking-[0.3em] text-teal-400 font-semibold">Propietarios</p>
          ...
          <Link
            href="/propietarios"
            className="mt-8 inline-flex rounded-full bg-teal-600 px-8 py-3.5 text-sm font-semibold text-white shadow-lg transition hover:bg-teal-500"
          >
            Publicar mi casa
          </Link>
        </div>
