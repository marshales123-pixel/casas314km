    ...
    <main className="bg-white text-gray-800">
      ...
      <section className="border-b border-stone-100 bg-stone-50 py-16">
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
          <p className="text-xs uppercase tracking-[0.25em] text-teal-600 font-medium">Propietarios</p>
      ...
      <section className="mx-auto max-w-4xl px-4 py-16 sm:px-6 lg:px-8">
        <h2 className="text-xl font-semibold text-stone-900 mb-8">¿Qué incluye el servicio?</h2>

        <div className="grid gap-5 sm:grid-cols-2">
          {beneficios.map(({ titulo, descripcion, icon }) => (
            <div
              key={titulo}
              className="flex gap-4 rounded-2xl border border-stone-100 bg-stone-50 p-6 transition hover:border-teal-100 hover:bg-teal-50/30"
            >
              <span className="text-2xl shrink-0">{icon}</span>
              <div>
                <h3 className="font-semibold text-stone-900">{titulo}</h3>
                <p className="mt-1 text-sm leading-6 text-stone-500">{descripcion}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <section className="bg-stone-900 py-16">
        <div className="mx-auto max-w-4xl px-4 text-center sm:px-6 lg:px-8">
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="mt-8 inline-flex items-center gap-2 rounded-full bg-teal-600 px-8 py-3.5 text-sm font-medium text-white shadow-lg transition hover:bg-teal-500"
          >
